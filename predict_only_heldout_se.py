...
def main(
    ...
    # ===== [PREDICT-ONLY] bat buoc phai co checkpoint =====
    if not os.path.exists(resume_path):
        raise FileNotFoundError('[PREDICT-ONLY] Khong tim thay {} — can checkpoint de predict.'.format(resume_path))
    if True:
        try:
            ckpt = torch.load(resume_path, map_location=device)
            model.load_state_dict(ckpt['model_state_dict'])
            # [PREDICT-ONLY] bo qua optimizer (khong train)
            start_epoch = int(ckpt.get('epoch', 0))
            print('[PREDICT-ONLY] fold {}: da load checkpoint (epoch {})'.format(k, start_epoch))
    ...
    for epoch in range(num_epoch, num_epoch):  # [PREDICT-ONLY] khong train
    ...
    if False:  # [PREDICT-ONLY] khong ghi de checkpoint
