...
print(f'=== Doc predictResult/{prefix}total_*_k.npy (k=0..{n_folds-1}) ===')
for k in range(n_folds):
    try:
        labels = np.load(f'predictResult/{prefix}total_labels_{k}.npy')
        preds = np.load(f'predictResult/{prefix}total_preds_{k}.npy')
