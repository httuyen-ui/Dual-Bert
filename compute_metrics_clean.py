...
print(f'=== Doc predictResult_clean/{prefix}total_*_k.npy (k=0..{n_folds-1}) ===')
for k in range(n_folds):
    try:
        labels = np.load(f'predictResult_clean/{prefix}total_labels_{k}.npy')
        preds = np.load(f'predictResult_clean/{prefix}total_preds_{k}.npy')
